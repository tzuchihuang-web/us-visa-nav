...
export function RecommendedPathPanel({
  ...
  return (
    <div 
      className={`absolute bottom-4 left-4 right-4 glass-panel border-t-2 ${config.border} p-4 ${className} shadow-2xl z-50`}
      style={{
        maxHeight: '32vh',
        overflowY: 'auto',
      }}
    >
      <div className="max-w-7xl mx-auto">
        ...
        {/* Description */}
        <p className="text-xs text-gray-700 mb-3 font-semibold">{path.description}</p>
          ...
          {path.steps.map((step, index) => (
            <React.Fragment key={step.visaId}>
              ...
              <button
                ...
                className={`
                  flex-shrink-0 p-3 glass-panel rounded-xl border-2 transition-all
              ...
              >
                <div className="flex items-start gap-3">
                  <div className="flex-1 text-left">
                    {/* Step Number */}
                    <div className="text-xs font-black text-gray-500 mb-1 tracking-wider">
                      STEP {index + 1}
                    </div>

                    {/* Visa Name */}
                    <div className="font-black text-black mb-1 text-sm">
                      {step.visa.name}
                    </div>
